...
from util import *
import random
from model_LLM_TFP import LLM_TFP
...
parser.add_argument(
    "--save",
    type=str,
    default="./logstest/" + str(time.strftime("%Y-%m-%d-%H:%M:%S")) + "-",
    help="save path",
)
parser.add_argument(
    "--es_patience",
...
def main():
    ...
    for i in range(1, args.epochs + 1):
        ...
        if epochs_since_best_mae >= args.es_patience and i >= 150:
            break
    ...
    # engine.model.load_state_dict(torch.load("./logsPara/pems-15min/best_model.pth"))
    outputs = []
    realy = torch.Tensor(dataloader["y_test"]).to(device)
    realy = realy.transpose(1, 3)[:, 0, :, :]
